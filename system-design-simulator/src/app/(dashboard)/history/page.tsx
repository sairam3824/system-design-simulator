...
export default async function HistoryPage() {
  ...
  const [interviews, codingChallenges] = await Promise.all([
    ...
    prisma.codingChallenge.findMany({
      where: { userId },
      orderBy: { createdAt: "desc" },
      include: { score: true, problem: true },
    }),
  ]);
                ...
                <div className="divide-y divide-border/50">
                  {interviews.map((interview) => (
                    <Link
                      ...
                      className="block group hover:bg-muted/30 transition-colors"
                    >
                          ...
                          {challenge.score ? (
                            <>
                              <div className="flex-1 space-y-1.5">
                                ...
                                <Progress
                                  value={challenge.score.overallScore}
                                  className="h-2"
                                />
                              </div>
                              <Badge

...
"[project]/Documents/Git/system-design/system-design-simulator/src/app/api/coding/analytics/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        const avgOverallScore = scoresWithValues.length > 0 ? scoresWithValues.reduce((acc, c)=>acc + (c.score?.overallScore || 0), 0) / scoresWithValues.length : 0;
        const avgCorrectness = scoresWithValues.length > 0 ? scoresWithValues.reduce((acc, c)=>acc + (c.score?.correctness || 0), 0) / scoresWithValues.length : 0;
        const avgEfficiency = scoresWithValues.length > 0 ? scoresWithValues.reduce((acc, c)=>acc + (c.score?.efficiency || 0), 0) / scoresWithValues.length : 0;
        const avgCodeQuality = scoresWithValues.length > 0 ? scoresWithValues.reduce((acc, c)=>acc + (c.score?.codeQuality || 0), 0) / scoresWithValues.length : 0;
        const avgEdgeCases = scoresWithValues.length > 0 ? scoresWithValues.reduce((acc, c)=>acc + (c.score?.edgeCases || 0), 0) / scoresWithValues.length : 0;
        // Category breakdown
        const categoryStats = {};
        ...
        const sortedDimensions = Object.entries(dimensionScores).sort((a, b)=>a[1] - b[1]);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__17a1cf99._.js.map